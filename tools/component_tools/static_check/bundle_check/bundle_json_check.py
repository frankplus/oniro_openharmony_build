...
def check_all_bundle_json(path:str) -> dict:
    '''
    ...
    if os.path.isabs(path):
        ...
        target_path = os.path.join(g_root_path, os.path.normpath(path))
...
class BundlesCheck:
    ...
    def to_json(all_errors:dict,
        ...
        all_errors = check_all_bundle_json(g_root_path)
    ...
    def to_df(path:str = None) -> pd.DataFrame:
        ...
        if path is None:
            ...
            path = os.path.join(g_root_path, path)
                ...
                for bundle_path in all_errors[subsystem][component]:
                    for rule in all_errors[subsystem][component][bundle_path]:
                        for error_item in all_errors[subsystem][component][bundle_path][rule]:
                            ...
                            error_list = list([subsystem, component, bundle_path, rule, content, desc])
...
class BundleJson(object):
    ...
    def check_name(self) -> dict:
        ...
        if not match:
            bundle_error["description"] = NAME_FORMAT_ERROR + \
                COMPONENT_NAME_FROMAT + COMPONENT_NAME_FROMAT_LEN
            return bundle_error
        return None
    ...
    def check_version(self) -> dict:
        ...
        if self.__json['version'] != g_ohos_version:
            bundle_error['description'] = VERSION_ERROR + ' current ohos version is: ' + g_ohos_version
            return bundle_error
        return None
    ...
    def check_component(self) -> list:
        ...
        if 'syscap' not in component:
            ...
            for i in component['syscap']:
                ...
                if not i:
                    ...
                    continue
                match = re.match(r'^SystemCapability(\.[A-Z][a-zA-Z]+)+$', i)
                if not match:
        ...
        if 'features' not in component:
            ...
            for feature in component["features"]:
                ...
                if not match:
                    err.append(COMPONENT_FEATURES_FORMAT_ERROR)
                else:
