...
class GiteeCsctPrehandler:
    def __init__(self, pr_list: str, *patterns):
        ...
        repo_prNum_list = pr_list.split(';')
        for pr_item in repo_prNum_list:
            pr_split_group = pr_item.split("pulls/")
            repo_url = pr_split_group[0]
            pr_num = pr_split_group[1]

            gitee_pr_diff = get_diff_by_repo_prNum(repo_url, pr_num)
