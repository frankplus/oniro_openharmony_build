...
def desc_check(content):
    if ((not length_check(content, 3, 128))
        or (not check_char_desc(content))):
        sys.exit(1)
        ...
        for param_name, param_value in param_info.items():
            ...
            if "arrsize" in param_value:
                if ((param_value["arrsize"] < 1)
                    ...
                    print("error: ", param_name, "array size must be in range [1, 100]")
                    sys.exit(1)
...
def merge_hisysevent_config(yaml_list, output_path):
    ...
    dictionary_result = {}
    global WARNING_TXT
    hisysevent_warning_file = os.path.join(output_path, 'hisysevent_warning.txt')
    ...
    print("Verify the YAML files and pack them into {} hisysevent.def successful".format(hisysevent_def_file))
...
def main(argv):
    ...
    hisysevent_def_file = merge_hisysevent_config(args.yaml_list, args.def_path)
