...
class Gn(BuildFileGeneratorInterface):
    ...
    def _check_options_validity(self, option: str, args_file: dict):
        support_sub_options = args_file.get("argAttribute").get("support_sub_options")
        option_name = option.lstrip('-')
        option_value = ""
        if '=' in option:
            option_name, option_value = option.lstrip('-').split('=')
        if option_name in support_sub_options:
            sub_optional_list = support_sub_options.get(option_name).get("argAttribute").get("optional")
            ...
            raise OHOSException('ERROR argument "{}": Invalid choice "{}". '
                                    'choose from {}'.format(args_file.get("argName"), option, list(support_sub_options.keys())), '3003')
        ...
        for arg in kwargs.get("args_list"):
            if(arg.endswith('.gn')):
               if (os.path.exists(arg)):
                   ...
                   raise OHOSException("ERROR Couldn't read '{}'".format(arg), '3005')
            else:
                if arg.startswith('-'):
                    self._check_options_validity(arg, args_file)
                gn_format_cmd.append(arg)
